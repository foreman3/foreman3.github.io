...
(async () => {
  ...
  try {
    ...
    const switchControls = async target => {
      await target.locator('.vibecade-mobile-options').click();
      await target.locator('.vibecade-control-toggle').click();
      await target.locator('.vibecade-options-close').click();
    };
    assert.equal(await toggle.isVisible(),false);
    assert.equal(await page.locator('[data-joystick] .vibecade-control-toggle').count(),0);
    ...
    await press('left',7);await switchControls(page);assert.deepEqual(await vector(),[0,0]);assert.equal(await page.locator('.vibecade-direction-pad').isVisible(),false);
    ...
    await switchControls(other);await page.waitForFunction(()=>document.querySelector('[data-joystick]').dataset.controlStyle==='joystick');
    ...
    await page.goto(base+'/controls-test?mode=cardinal');await switchControls(page);await press('left',8);await press('up',9);assert.deepEqual(await vector(),[0,-1]);await release(9);assert.deepEqual(await vector(),[-1,0]);await release(8);
    ...
    for(const [width,height] of [[667,375],[740,390],[844,390]]){
      ...
      const gear=await page.locator('.vibecade-mobile-options').boundingBox(),reset=await page.locator('.vibecade-mobile-restart').boundingBox();
      assert(gear.x+gear.width<reset.x && gear.y===reset.y && gear.x>=board.x+board.width);
      await page.locator('.vibecade-mobile-options').click();
      const popup=await page.locator('#vibecade-options').boundingBox();assert(popup.x>=0&&popup.y>=0&&popup.x+popup.width<=width&&popup.y+popup.height<=height,JSON.stringify({popup,width,height,gear,reset}));
      await page.keyboard.press('Escape');assert.equal(await toggle.isVisible(),false);
      await page.locator('.vibecade-mobile-options').click();await page.mouse.click(2,height-2);assert.equal(await toggle.isVisible(),false);
      for(const selector of ['.vibecade-direction-pad']){const box=await page.locator(selector).boundingBox();assert(box.x>=0&&box.y>=0&&box.x+box.width<=board.x+1&&box.y+box.height<=height,JSON.stringify({width,height,selector,box,board}));}
    }
    await page.evaluate(()=>unbind());assert.equal(await page.locator('.vibecade-mobile-options').isVisible(),false);assert.deepEqual(await vector(),[0,0]);
    const desktop=await browser.newPage({viewport:{width:1440,height:900}});await desktop.goto(base+'/controls-test');assert.equal(await desktop.locator('.vibecade-mobile-options').isVisible(),false);await desktop.close();
    const blocked=await browser.newContext({viewport:{width:740,height:390}});await blocked.addInitScript(()=>{Storage.prototype.getItem=()=>{throw Error('blocked')};Storage.prototype.setItem=()=>{throw Error('blocked')};});const bp=await blocked.newPage();await bp.goto(base+'/controls-test');await switchControls(bp);assert.equal(await bp.locator('[data-joystick]').getAttribute('data-control-style'),'buttons');await blocked.close();

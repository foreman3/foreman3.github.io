...
const briefs=['Three rubies, then reach the EXIT.','Two chambers. Cross the passages to steal from both.','Aimed beams: leave the striped line before it fires.','Shutters alternate. Cross the OPEN passage; watch the countdown.','Beams + shutters. Spend your tail to survive, or risk a bonus.'];
...
function refresh(){
 ...
 if(v.beam&&beam)security+=' · '+(grace>0?'BEAM JAMMED':beam.stage==='warning'?'BEAM IN '+(beam.left/pressure()).toFixed(1)+'s':beam.stage==='active'?'BEAM LIVE':'BEAM RECHARGING');
...
function startVault(n){
 level=n;const v=vaults[n-1];gems=0;bonus=0;unlocked=false;alarm=0;securityClock=0;gateClock=0;openGate=0;gateHeld=[false,false];beam=null;shedCooldown=0;grace=0;
...
function crash(reason='Collision'){
 lives--;flash=.6;tone(130,.2);if(lives<=0){finish(false);refresh();return;}
...
function tick(){
 ...
 if(beamHits(head)){crash('Security beam');return;}
 snake.unshift(head);
...
function updateSecurity(dt){
 ...
 if(v.beam&&beam){beam.left-=dt*pressure();if(beam.left<=0){if(beam.stage==='warning'){beam.stage='active';beam.left=level===5?1.1:.8;tone(170,.12);}else if(beam.stage==='active'){beam.stage='idle';beam.left=level===5?1.7:2.8;}else armBeam();}}
}
function shed(){
 ...
 if(beam){beam.stage='idle';beam.left=1.6;}announce('Tail shed −'+cost+' · security grace 1.4s · haul kept');tone(330,.2);refresh();dirty=true;
}

function step(dt){
 if(phase!=='playing')return;if(document.body.classList.contains('touch-device')&&innerWidth<600&&innerHeight>innerWidth)return;
 elapsed+=dt;effects.forEach(e=>e.life-=dt);effects=effects.filter(e=>e.life>0);flash=Math.max(0,flash-dt);
 ...
 if(beamHits(snake[0])){crash('Security beam');return;}
...
function dismiss(){try{sessionStorage.setItem('velvet-coil-heist-intro','1');}catch(_){}$('instruction-modal').classList.remove('is-visible');document.body.classList.remove('instructions-open');phase=resumePhase;last=performance.now();canvas.focus({preventScroll:true});dirty=true;}
...
function drawSecurity(){
 ...
 const active=beam.stage==='active'&&grace<=0,position=beam.line*CELL;
 ctx.save();ctx.beginPath();ctx.rect(OX,OY,COLS*CELL,ROWS*CELL);ctx.clip();ctx.fillStyle=active?'#ec66785e':'#e9b56520';
 if(beam.axis==='row')ctx.fillRect(OX,OY+position,COLS*CELL,CELL);else ctx.fillRect(OX+position,OY,CELL,ROWS*CELL);
 ctx.strokeStyle=active?'#ffc3c3':'#efc476';ctx.lineWidth=active?5:2;ctx.setLineDash(active?[]:[8,8]);
 const a=beam.axis==='row'?[[OX,OY+position+18],[OX+COLS*CELL,OY+position+18]]:[[OX+position+18,OY],[OX+position+18,OY+ROWS*CELL]];path(a,ctx.strokeStyle,ctx.lineWidth);ctx.restore();
 const bx=beam.axis==='row'?OX+12:OX+position+18,by=beam.axis==='row'?OY+position+24:OY+20;text(active?'!':'!',bx,by,22,'#fff0bc');
 }
}
...
if(params.has('test'))window.coilTest={state:()=>({level,score,gems,lives,time,charge,snake:snake.map(p=>({...p})),gem:gem&&{...gem},dir,queue:[...queue],grow,phase,recovery,slow,elapsed,acc,vaults,pillars,unlocked,bonus,alarm,beam:beam&&{...beam},gateClock,openGate,gateHeld:[...gateHeld],gatePeriod:gatePeriod(),shedCooldown,grace,exitCell}),seed:n=>seed=n>>>0,jump:n=>startVault(n),step,turn,crash,draw,shed,setTime:t=>time=t,setGem:p=>gem=p,setSnake:(a,d)=>{snake=a;dir=d;grow=0;queue=[];recovery=0;acc=0;},setGems:n=>{gems=n;unlocked=n>=vaults[level-1].quota;},setBeam:b=>beam=b,setGate:t=>gateClock=t,setScore:n=>score=n};

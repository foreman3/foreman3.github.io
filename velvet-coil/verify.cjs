...
(async()=>{
 ...
 await page.evaluate(playVault,{level:3,stopAt:2});await page.screenshot({path:path.join(out,'velvet-coil-beams.png')});await page.evaluate(playVault,{level:4,stopAt:3});await page.screenshot({path:path.join(out,'velvet-coil-heist.png')});
 for(const [width,height] of [[667,375],[740,390],[844,390],[390,844]]){
  ...
  await mp.evaluate(playVault,{level:4,stopAt:2});await mp.screenshot({path:path.join(out,`heist-${width}x${height}.png`)});await mp.reload();await mp.locator('.vibecade-mobile-play').click();await mp.waitForFunction(()=>document.documentElement.dataset.vibecadeLaunch==='playing');await mc.close();
 }

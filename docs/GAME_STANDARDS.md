...
The shared helper puts the mobile **Joystick / Buttons** preference inside an **Options** popup, opened by a gear beside Reset in the upper right. Reserve the popup for current and future settings; do not place preference toggles beside gameplay controls. It saves `vibecade-mobile-controls` in local storage, so the preference follows the player across games and reloads on the same browser and origin. Horizontal joysticks offer left/right buttons; cardinal and analog joysticks offer four directions. Analog buttons allow simultaneous directions with normalized diagonal speed; cardinal buttons keep one direction active. Keep this behavior in the shared helper rather than adding per-game preferences. Switching modes, releasing or cancelling a pointer, losing capture, hiding the page, restarting, and window blur must clear held input. The Options button inherits instruction visibility; direction buttons stay in the control rail.
